;(function(__root) {
/*
	Matreshka v1.1.0-alpha.1 (2015-09-13)
	JavaScript Framework by Andrey Gubanov
	Released under the MIT license
	More info: http://matreshka.io
*/
var matreshka_dir_xclass, matreshka_dir_polyfills_addeventlistener, matreshka_dir_balalaika, matreshka_dir_polyfills_classlist, matreshka_dir_balalaika_extended, matreshka_dir_dollar_lib, matreshka_dir_binders, matreshka_dir_matreshka_magic, matreshka_dir_matreshka_core, matreshka_dir_matreshka_object, matreshka_dir_matreshka_array, matreshka, balalaika, xclass, matreshka_magic;
...
matreshka_dir_binders = function (MK) {
...
matreshka_dir_matreshka_magic = function ($b, $, binders) {
  // make dollar! useas$
  // trim, extend, randomString, toArray, each,
  // review and resort
  var magic, toArray, each, extend, trim, sym, initMK = function (object) {
    ...
    selectNodes = function (object, s) {
      var result = $(), execResult, $bound, node, selectors, selector, i, j, random;
  ...
  magic = {
    ...
    domEvents: {
      ...
      remove: function (o) {
        var evts = this.list[o.instance[sym].id], evt, i;
        if (!evts)
          return;
    ...
    on: function (object, names, callback, triggerOnInit, context, evtData) {
      ...
      names = names instanceof Array ? names : trim(names).replace(/\s+/g, ' ')  // single spaces only
...
;
    ...
    _addListener: function (object, name, callback, context, evtData) {
      ...
      allEvents['addevent:' + name] && magic._trigger(object, 'addevent:' + name);
      return object;
    },
    ...
    off: function (object, names, callback, context) {
      ...
      names = trim(names).replace(/\s+/g, ' ')  // single spaces only
.split(/\s(?![^(]*\))/g);
    ...
    _trigger: function (object, name) {
      var events = object && typeof object == 'object' && object[sym] && object[sym].events && object[sym].events[name], args, i, l, ev;
      if (events) {
        args = toArray(arguments, 2), i = -1, l = events.length;
        while (++i < l)
    ...
    selectAll: function (object, s) {
      if (!object || typeof object != 'object' || !object.$sandbox)
        return $();
      initMK(object);
    ...
    boundAll: function (object, key) {
      if (!object || typeof object != 'object')
        return $();
      initMK(object);
      var special = object[sym].special, keys, $nodes, i;
      key = !key ? 'sandbox' : key;
    ...
    _getNodes: function (object, s) {
      return typeof s == 'string' && !/</.test(s) && /:sandbox|:bound\(([^(]*)\)/.test(s) ? selectNodes(object, s) : $(s);
    },
    ...
    lookForBinder: function (node) {
      var result, ep = magic.defaultBinders, i;
  ...
  return magic;
}(matreshka_dir_balalaika_extended, matreshka_dir_dollar_lib, matreshka_dir_binders);
...
matreshka_dir_matreshka_core = function (Class, magic) {
  ...
  return MK;
}(matreshka_dir_xclass, matreshka_dir_matreshka_magic);

matreshka_dir_matreshka_object = function (MK) {
  if (!MK) {
  ...
  var sym = MK.sym, i, prototype = {
      ...
      */
      ...
      jset: function (key, v, evt) {
        var _this = this._initMK(), type = typeof key;
        if (type == 'undefined')
          return _this;
        if (key && type == 'object') {
...
}(matreshka_dir_matreshka_core);

matreshka_dir_matreshka_array = function (MK) {
  if (!MK) {
    throw new Error('Matreshka is missing');
  }
  var Array_prototype = Array.prototype, sym = MK.sym, toArray = MK.toArray, slice = Array_prototype.slice, isXDR = MK.isXDR, compare = function (a1, a2, i, l) {
      ...
      return -1;
    } : Array_prototype.indexOf, lastIndexOf = isXDR ? function (sought) {
      var _this = this, l = _this.length, i, item, isMK = sought && sought.isMK;
      for (i = l - 1; i >= 0; i--) {
        item = _this[i];
        if (isMK ? sought.eq(item) : sought === item) {
      ...
      return -1;
    } : Array_prototype.lastIndexOf, triggerModify = function (_this, evt, additional) {
      ...
      case 'reverse':
        return function () {
          if (!this.length)
            return;
          var _this = this._initMK(), _arguments = arguments, args = toArray(_arguments), evt = hasOptions ? _arguments[_arguments.length - 1] || {} : {}, array = _this.toArray(), returns = Array_prototype[name].apply(array, args);
          if (hasOptions) {
            args.pop();
          }
          if (isXDR) {
      ...
      */
      _renderOne: function (item, evt) {
        if (!item || typeof item != 'object' || !this.renderIfPossible || evt.dontRender)
          return;
        if (!item[sym]) {
          item._initMK ? item._initMK() : MK.initMK(item);
        }
        var _this = this, id = _this[sym].id, renderer = item.renderer || _this.itemRenderer, rendererContext = renderer === item.renderer ? item : _this, arraysNodes = item[sym].arraysNodes = item[sym].arraysNodes || {}, node = arraysNodes[id], $node, template, itemEvt, sandboxes, i;
        if (!renderer)
          return;
        if (evt.moveSandbox) {
      ...
      processRendering: function (evt) {
        var _this = this, props = _this[sym], id = props.id, l = _this.length, getArrayNode = function (item) {
            var arraysNodes;
            if (item && item.isMK) {

 ...
 * todo silentSort
 * todo trigger 'modify' on createSelf (maybe rename to create and make silentCreate)
 * todo add method that removes array item and change next indexes (splice one given element)
 * todo renderer (DOMCollection)
 */
( function( MK, Array_prototype ) {
	...
	var	createArrayMethod = function( type, name, silent ) {
		if( type === SIMPLE ) {
			...
			return function() {
				var array = this.toArray(),
					returns = Array.prototype[ name ].apply( array, arguments );
					return new MK.Array().createSelf( returns );
			};
			...
			return function() {
				var array = this.toArray(),
					returns = Array.prototype[ name ].apply( array, arguments );
				this.createSelf( array );
				returns = new MK.Array().createSelf( returns );
				if( !silent ) {
					this.trigger( name, {
						returns: returns,
						method: name,
	...
	MK.Array = Class({
		...
		constructor: function( length ) {
			...
			this.on( 'push pop unshift shift splice sort remove', function( evtOptions ) {
				this.trigger( 'modify', evtOptions );
			});
		},
		/**
		 * @method Matreshka.Array#createSelf
		 * @summary Creates self from another array 
		 * @desc If you have array or array-like object (eg arguments) you can convert it to MK.Array instance by the method
		 * @param {Array} array
		 * @returns {mkArray} self
		 * @example <caption>Basic usage</caption>
		 * new MK.Array().createSelf( [1, 2, 3, 4, 5] );
		 */
		createSelf: function( array ) {
		 ...
		 * @returns {*}
		 * @example <caption>Basic usage</caption>
		 * new MK.Array().createSelf( [0,1,2,3,4] ).reduce( function( previousValue, currentValue ) {
		 ...
		 * @returns {*}
		 * @example <caption>Basic usage</caption>
		 * new MK.Array().createSelf( [0,1,2,3,4] ).reduceRight( function( previousValue, currentValue ) {
		 * 	return previousValue + currentValue;
		 * });
		 */
		 ...
		 * @returns {string}
		 * @example <caption>Basic usage</caption>
		 * var mkArray = new MK.Array().createSelf( [ 1, 2, 3, 4, 4, 3, 2, 1 ] );
